...
interface RunsHistoryListProps {
  maxItems?: number;
  hideHeader?: boolean;
}

export default function RunsHistoryList({ maxItems, hideHeader }: RunsHistoryListProps) {
  ...
  useEffect(() => {
    async function fetchRuns() {
      try {
        ...
        let mappedRuns = response.data.map((run: any) => {
        ...
        if (maxItems) {
          mappedRuns = mappedRuns.slice(0, maxItems)
        }
    ...
    <div className="space-y-6">
      {!hideHeader && (
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-black text-landing-primary tracking-tight">Archive Data</h2>
